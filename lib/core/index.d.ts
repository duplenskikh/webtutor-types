 ...
 * @param {string} arg1 - код, который необходимо выполнить.
 ...
 * @param {string} name - название журнада.
 * @param {string} options - набор опций вида 'param1=value1;param2=value2;...'. Необязательный аргумент.
 * @returns undefined
 * EnableLogExt( 'web-request', 'header-str=date\turl\tquery;use-std-event-prefix=0' ) Возможные опции: life-time - период, на котрый заводится новый файла журнала ('day', 'month', 'permanent' ). По умолчанию свой файл журнала заводится на каждую дату ('day') base-dir - директория, в которой будут заводиться журнал. По умолчанию используется директория Logs в установочной директории use-std-event-prefix - Включать в начало каждой строки журнала стандартный префикс (дата, время, ID потока) (Bool). По умолчанию true. header-str - строка заголовка, добавляемая в начало каждого нового файла журнала. По умолчанию строка заголовка не добавляется.
 */
 ...
 * @returns number
 * work-dir - рабочая директория для процесса (String) sys - cсистемный процесс (Bool),
 * в этом случае в качестве первого аргумента функции можно указывать не путь до исполняемого файла,
 * а имя системного процесса wait - дожидатсья завершения процесса (Bool). Если указана опция wait=1,
 * функция вернет код завершения процесса (0 - успешное завершение, иначе - ошибка).
 * В остальных случаях функция ничего не возвращает. hidden - запуск без пользовательского интерфейса (Bool)
...
/**
 * Ищет первый элемент массива с заданным значением определнного поля (ключа).
 * Если такой элемент не найден, возвращается undefined.
 * @param {Array} array - массив
 * @param {any} keyValue - значение ключа.
 * @param {string} keyName - имя элемента, являющегося ключом. Необязательный аргумент. Если имя ключа не указано, используется первичный ключ.
 * @returns {any}
 */
 ...
 * @param {Array} array - исходный массив
 * @param {string} fieldExpr - выражение, вычисляемое относительно каждого элемента исходного массива. Необязательный аргумент. Если аргумент не указан, используется значение самого элемента (This).
 * @returns {Array}
 */
 ...
 * (режим терпимости к незаполенным и битым ссылкам).
 * @param {Array} array - массив объектов
 * @param {any} keyVal - значение ключевого элемента (any)
 * @returns any
 ...
 * @returns {Object}
 * @example catalog = FindSharedCatalog("events");
 */
declare function FindSharedCatalog(catalogName: string): any;

/**
 * Аналог функции {@link LoadUrlData}, позволящий явно указать адрес сервера приложения,
 * с котрого будут загружаться данные.
 ...
 * @returns {Object}
 * @example catalog = FindOptCatalog("events");
 */
 ...
 * Объект, информация из которого может быть загуржена в базу данных,
 ...
 * @returns {Array}
 * lds-server - явный адрес сервера приложения, на который будт отправлен запрос.
 * Используется для обмена данных между серверами.
 * preload-foreign-data - заранее кэшировать связанные данные (Bool).
 * Обычно используется при показе списков в пользовательском интерфейсе
 * сетевой версии, в которых будет использовать связянные данные (ForeignElem).
 * @example
 * ```
 ...
 * @returns {string}
 * @example StrLeftCharRange(str, length);
 */
 ...
 * @param {string} filePath - путь файловой системы.
 * @param {string} baseUrl - базовый url, к схеме которого будет приводиться путь. Необязательный аргумент.
 * @returns {string}
 * @xample FilePathToUrl("C:\\Temp\\1.htm") == "file:///C:/Temp/1.htm"
 */
...
/**
 * Извлекает из объекта типа Error пользовательскую часть сообщения об ошибке.
...
/**
 * Выполняет код JScript в текущем окружении. В отличие от функции eval() выполяняет код на том же уровне
 ...
 * @param {Array[any]} params (необязательный). Тип: Массив без описания структуры элементов. Массив параметров.
 ...
 * @param {any[]} (необязательный) - Объект, содержащий набор свойств, определяющих окружение, в котором будет исполнятся код.
 ...
 */
declare function OptEval(code: string, defaultValue?: any): any;
 ...
 * @returns WTXmlDocument
 */
declare function OpenCodeLib(url: string): any;
