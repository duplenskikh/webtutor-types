interface XmElem<T, ForeignElem = never> {
   ...
   * Конструкция `elem.Size` возвращает тот же результат, что и `StrLen(elem)` но, в отличие от последней,
   ...
   * Поля типа str`ing становятся пустыми строками,
   ...
   * Массивы сравниваются поэлементно, при этом требуется совпадение как количества лементов в массиве так и порядка их следования.
  ...
  /**
   * Устанавливает значение атрибута с заданным именем, если значение атрибута отличается от его требуемого значения по умолчанию.
   * Метод работает только для динамических элементов.
   ...
   * @param {string} name - Имя атрибута.
   * @param {string} value - Значение атрибута.
   * @param {string} defaultValue - Значение атрибута по умолчанию. Необязательный аргумент. По умолчанию - пустая строка.
   */
